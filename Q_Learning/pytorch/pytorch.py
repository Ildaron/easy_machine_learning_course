#https://github.com/zzzxxxttt/pytorch_simple_RL/blob/master/a2c_cartp.py
import gym
import time
import numpy as np
import argparse
...
def main():
  for episode in range(cfg.max_episode):
    ...
    for episode_steps in count():
      print ("ok")
      print ("state", state)   
      action = get_action(torch.tensor([state]).float()[None, :]).item() #IndexError: too many indices for tensor of dimension 0
      print ("ok1") 
      next_state, reward, done, _ = env.step(action)
      #asd=[state,action,next_state,reward]   
